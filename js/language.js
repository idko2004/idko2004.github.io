...
const allTexts =
{
	...
	'mbatterydiscord1': {es: "Si es que tienes un dispositivo con Android y utilizas ese dispositivo como algún tipo de servidor gracias a Termux, no quieres tenerlo enchufado todo el rato para que no se le arruine la batería, pero también se te suele olvidar enchufarlo cuando ya le queda poca, este proyecto podría ser de ayuda.", en: "If you happen to have an Android device that you're using as some kind of server with Termux and you don't want for it to be plugged in all the time, but also you always forget to charge it, maybe this project can help."},
	'mbatterydiscord2': {es: "envía un mensaje de Discord cuando la batería del dispositivo está baja. Así que si tienes activas las notificaciones de Discord, deberías enterarte cuando el bot te hace un ping. ¡Así ya no se le acaba la batería!", en: "sends a Discord message when the battery of the device is low. So if you have Discord notifications active, you should receive a ping from the bot."},
	'mbatterydiscord3': {es: "no se ejecuta en segundo plano, sino que comprueba si la batería está baja, envía un mensaje si es necesario y se cierra. Por lo que tendrás que configurar un cronjob en Termux, en el readme del proyecto hay instrucciones sobre cómo hacer eso. También es necesario crear tu propio bot de Discord.", en: "does not execute in the background, instead, it checks the battery level, sends a message if necessary and exits. So you'll have to configure a cronjob on Termux, there are instructions on the project's readme on how to do so. Also you'll have to create your own Discord bot"},
